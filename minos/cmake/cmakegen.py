...
def write_cmake(release_base, makefiles):
  ...
  # Raw CMakeLists
  cmake_base = open(os.path.join(os.path.dirname(__file__), "data", "BaseCMakeLists.txt")).read()
  cmake_base = cmake_base.replace("$subdirectories$", "\n".join("add_subdirectory({})".format(x) for x in makefiles.keys()))
  ...
  liblookup = {_get_libname(x): x.target for x in _flatten_makefiles(makefiles.values()) if x.build_library}

  for package, make in makefiles.items():
    write_package_cmakelist(os.path.join(release_base, package), make, liblookup)